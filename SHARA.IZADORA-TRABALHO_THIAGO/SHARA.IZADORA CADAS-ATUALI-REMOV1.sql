...
BEGIN
    -- Verificar se o produto componente não é um produto composto
    IF EXISTS (SELECT 1 FROM COMPOSICAO_PRONTA WHERE ID_PROD_COMP1 = P_ID_PROD_COMP2 AND ATIVO = TRUE) THEN
        RETURN 'ERRO: UM PRODUTO COMPOSTO NÃO PODE DEPENDER DE OUTRO PRODUTO COMPOSTO.';
    END IF;
