...
BEGIN
    ...
    FOR COMPONENTE IN
        SELECT CP.ID_PROD_COMP2, CP.QUANT * P_QUANTIDADE AS QUANT_NECESSARIA,
               CP.PERDA, P.TIPO
    ...
    LOOP
        ...
        IF COMPONENTE_EH_COMPOSTO THEN
            TEM_ESTOQUE := VERIFICAR_ESTOQUE_COMPOSICAO_RECURSIVO(
                ...
                COMPONENTE.QUANT_NECESSARIA
            );
        ELSE
            TEM_ESTOQUE := (SELECT ESTOQUE >= COMPONENTE.QUANT_NECESSARIA
                            FROM PRODUTO
...
BEGIN
    ...
    WHERE P.ATIVO = TRUE AND I.ATIVO = TRUE AND PE.ATIVO = TRUE
...
BEGIN
    ...
    WHERE
        F.ATIVO = TRUE AND (P.ID_PEDIDO IS NULL OR P.ATIVO = TRUE)
    GROUP BY
